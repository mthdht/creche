...
import { type Child, type BreadcrumbItem, type Daycare, type Transmission } from '@/types';
...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    },
    {
        ...
        href: route('daycares.children.transmissions.edit', {daycare: props.daycare.id, child: props.child.id, transmission: props.transmission.id})
    },
];

const form = useForm({
    type: props.transmission.type,
    description: props.transmission.description,
    severity: props.transmission.severity,
    duration: props.transmission.duration,
    datetime: props.transmission.datetime
});

const submit = () => {
    form.put(route('daycares.children.transmissions.update', {daycare: props.daycare.id, child: props.child.id, transmission: props.transmission.id}));
};

</script>

<template>
    <Head title="Modification d'une activité" />
        ...
        <div class="space-y-4">
            <h2 class="font-semibold text-xl flex justify-between gap-3">
                <div class="flex gap-3">
                    ...
                    Modifier une activité à {{  props.child.first_name }}    
                </div>
            </h2>
